...
int save_members_to_file(const char* filename) {
...
double calculate_member_discount(const char* member_id) {
    for (int i = 0; i < member_count; i++) {
        if (strcmp(members[i].member_id, member_id) == 0) {
            // 根据积分计算折扣
            if (members[i].points >= 1000) return 0.1; // 10%折扣
            if (members[i].points >= 500) return 0.075;  // 7.5%折扣
